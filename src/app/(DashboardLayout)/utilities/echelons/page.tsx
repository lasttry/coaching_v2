...
const EchelonsPage = () => {
  ...
  const getGroupedAndSortedEchelons = (echelons: EchelonInterface[]) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForm({ id: null, minAge: null, maxAge: null, name: '', description: '', gender: null });
      fetchEchelons();
      setShowCancel(false);
    } catch (error) {
  ...
  // Handle edit
  const handleEdit = (echelon: EchelonInterface) => {
    setForm({
      id: echelon.id,
        ...
        <Grid container spacing={2}>
          ...
          <Grid size={{ xs: 12 }}>
            ...
            <Button
              ...
              variant="outlined"
              color="secondary"
              sx={{ marginLeft: 1, display: showCancel ? 'inline-flex' : 'none' }}
            >
              {t('cancel')}
            </Button>
          </Grid>
