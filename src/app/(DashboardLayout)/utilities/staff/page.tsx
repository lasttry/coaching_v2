...
export default function StaffPage(): React.JSX.Element {
  ...
  const handleSave = async () => {
    try {
      ...
      if (res.ok) {
        setSnackbar({
          open: true,
          message: selectedStaff ? t('staffUpdated') : t('staffCreated'),
          severity: 'success',
        });
        setDialogOpen(false);
        fetchStaff();
      ...
      setSnackbar({ open: true, message: t('errorSaving'), severity: 'error' });
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setSnackbar({ open: true, message: t('staffDeleted'), severity: 'success' });
      ...
      setSnackbar({ open: true, message: t('errorDeleting'), severity: 'error' });
    }
  };
  ...
  return (
    <Box sx={{ p: 3 }}>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2, mt: 1 }}>
            ...
            <FormControl fullWidth>
              <InputLabel>{t('teams')}</InputLabel>
              <Select
                multiple
                value={formData.teamIds}
                onChange={handleTeamChange}
                input={<OutlinedInput label={t('teams')} />}
      ...
      <Dialog open={deleteDialogOpen} onClose={() => setDeleteDialogOpen(false)}>
        ...
        <DialogContent>
          <Typography>{t('confirmDeleteStaff', { name: selectedStaff?.name })}</Typography>
        </DialogContent>
        <DialogActions>
