        ...
        if (playtimeResponse.ok) {
          ...
          const playtimeMap = playtimeData.reduce(
            (
              acc: Record<
                number,
                { totalTimePlayed: number; periods: Record<number, number> }
              >,
              entry: {
                athleteId: number;
  ...
  return (
    <PageContainer title="Game Details" description="Details of the game">
      ...
      <Box marginY={2}>
        ...
        <Typography variant="body1">
          <strong>Opponent:</strong> {game?.oponent?.name || 'N/A'}
        </Typography>
        <Typography variant="body1">
          ...
          game.gameAthletes.map((athletes) => (
            <Box key={athletes.athlete.id} marginY={1}>
              ...
              <Typography variant="body2" color="textSecondary">
                {Object.keys(playtime[athletes.athlete.id]?.periods || {}).map(
                  (period) => {
      ...
      <Stack direction="row" spacing={2} marginTop={4}>
        ...
        <Button
          ...
          color="primary"
          onClick={() =>
            game && settings && generateStatisticsPDF(game, settings)
          }
        >
        ...
        <Button onClick={() => game && generateReportsPDF(game)}>
