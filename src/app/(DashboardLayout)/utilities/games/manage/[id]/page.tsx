...
const GameFormPage = (props: { params: Params }) => {
  ...
  const handleAddObjective = () => {
    setObjectives([
      ...objectives,
      { title: '', description: '', type: ObjectiveType.TEAM },
    ]);
  };
  ...
  const sortAthletes = (athletes: GameAthleteInterface[]) => {
    return athletes.sort((a, b) => {
      const numberCompare =
        parseInt(a.athlete.number) - parseInt(b.athlete.number);
      if (numberCompare !== 0) return numberCompare;

      const yearCompare =
        dayjs(a.athlete.birthdate).year() - dayjs(b.athlete.birthdate).year();
  ...
  const handleAddAthlete = (athlete: GameAthleteInterface) => {
    const updatedSelected = [
      ...selectedAthletes,
      {
        ...athlete,
        number:
          athlete.athlete.number === '-1' ? '' : String(athlete.athlete.number),
      },
    ];
    const updatedAvailable = availableAthletes.filter(
      (a) => a.athlete.id !== athlete.athlete.id,
    );
  ...
  const handleRemoveAthlete = (athlete: GameAthleteInterface) => {
    ...
    const updatedSelected = selectedAthletes.filter(
      (a) => a.athlete.id !== athlete.athlete.id,
    );
    ...
    setForm((prev) => ({
      ...prev,
      gameAthletes: prev.gameAthletes.filter(
        (ath) => ath.athlete.id !== athlete.athlete.id,
      ),
    }));
  };
        ...
        <Stack spacing={3}>
          ...
          <Box>
            <Select
              ...
              onChange={(e) =>
                handleSelectChange(e as React.ChangeEvent<HTMLInputElement>)
              }
            >
                ...
                <Select
                  ...
                  onChange={(e) =>
                    handleObjectiveChange(
                      index,
                      'type',
                      e.target.value as ObjectiveType,
                    )
                  }
                  displayEmpty
                >
              ...
              <TableBody>
                ...
                {selectedAthletes.map((athlete) => (
                  <TableRow key={athlete.athlete.id}>
                        ...
                        {[1, 2, 3, 4].map((period) => (
                          <label key={`period-${period}`}>
                            <input
                              ...
                              checked={Boolean(
                                athlete[
                                  `period${period}` as keyof GameAthleteInterface
                                ],
                    ...
                    <TableCell>
                      {dayjs(athlete.athlete.birthdate).year()}
                    </TableCell>
