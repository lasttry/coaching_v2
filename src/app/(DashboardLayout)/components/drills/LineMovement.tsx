...
const LineMovement: React.FC<LineMovementProps> = ({
  id, points
}) => {
  ...
  return (
    <g>
      ...
      {console.log("ID:", id)}
      <path className={id} d={generatePathFromPoints(points)} fill="none" stroke={"black"} strokeWidth={2}
        markerEnd='url(#markerArrow)'
      />
      {selectedId === id && currentPoints.map((point, index) => (
        index % 2 === 0 && (
          <circle
            key={index}
            cx={point}
