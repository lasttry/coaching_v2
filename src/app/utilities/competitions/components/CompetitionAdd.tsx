    ...
    switch (field) {
      case 'name':
        if (typeof value !== 'string' || !value) {
          ...
          error = t('validation.nameMaxLength');
        }
        break;

      case 'description':
        if (typeof value === 'string' && value.length > 255) {
          error = t('validation.descriptionMaxLength');
        }
        break;
      ...
      case 'image':
        if (
          ...
          error = t('validation.imageInvalid');
        }
        break;

      case 'echelonId':
        if (typeof value !== 'number' || !value) {
          error = t('validation.echelonRequired');
        } else if (!Number.isInteger(value) || value <= 0) {
          error = t('validation.echelonIdPositive');
        }
        break;
    ...
    <>
      ...
      >
        <AccordionSummary expandIcon={<ExpandMoreIcon />}>
          <Typography>{t('competition.addNew')}</Typography>
        </AccordionSummary>
        <AccordionDetails>
