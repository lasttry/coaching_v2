...
export default function EntitiesPage(): React.JSX.Element {
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      ...
      setSuccessMessage(editingEntity ? t('status.updated') : t('status.created'));
      handleCloseDialog();
    } catch (err) {
  ...
  const handleDelete = async (): Promise<void> => {
    ...
    try {
      ...
      setSuccessMessage(t('status.deleted'));
  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        <Typography variant="h5">{t('common.management')}</Typography>
      ...
      <Dialog open={dialogOpen} onClose={handleCloseDialog} fullWidth maxWidth="sm">
        <DialogTitle>{editingEntity ? t('actions.edit') : t('actions.add')}</DialogTitle>
        <DialogContent>
