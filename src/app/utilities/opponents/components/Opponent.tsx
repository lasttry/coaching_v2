    ...
    <Grid container spacing={2}>
      <Grid size={{ xs: 12, sm: 12 }}>
        {imagePreview ? (
          ...
          <Typography>
            {t('common.imageNotSelected', { defaultValue: 'No image selected' })}
          </Typography>
        )}
      ...
      <Grid size={{ xs: 12, sm: 12 }}>
        <Typography variant="subtitle1">{t('venue.title')}</Typography>
        <Stack direction="row" sx={{ spacing: 1, mt: 1 }}>
          <TextField
            fullWidth
            label={t('opponent.newVenue')}
            value={newVenue}
            onChange={(e) => setNewVenue(e.target.value)}
          />
          <Button onClick={handleAddVenue}>{t('actions.add')}</Button>
        </Stack>
        ...
        {opponent?.venues?.length ? (
          ...
          <Typography variant="body2">{t('opponent.noVenues')}</Typography>
        )}
      </Grid>
    </Grid>
  );
