...
const OpponentListComponent: React.FC<OpponentListProps> = ({
  ...
  const handleSave = async (id: number): Promise<void> => {
    ...
    setSuccessMessage(t('opponent.save.updateSuccess'));
  ...
  const handleDelete = async (id: number): Promise<void> => {
    try {
      ...
      if (!res.ok) {
        ...
        const errorMessage = data?.error || t('opponent.save.deleteError');
      ...
      setSuccessMessage(t('opponent.save.deleteSuccess'));
      setTimeout(() => setSuccessMessage(null), 5000);
      setDeleteConfirm(null); // Close the dialog
    } catch (err) {
      setErrorMessage(t('opponent.save.deleteError'));
  ...
  return (
    <>
      <Dialog open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)}>
        ...
        <DialogContent>
          <DialogContentText>
            {t('messages.deleteConfirmation')} <strong>{deleteConfirm?.name}</strong>?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteConfirm(null)}>{t('actions.cancel')}</Button>
          <Button
                      ...
                      {isEditing ? (
                        ...
                        <Grid size={{ xs: 12 }}>
                          <Typography variant="body2">
                            {t('venues')}:{' '}
                            {opponent.venues?.map((v) => v.name).join(', ') || t('venue.none')}
                          </Typography>
                        </Grid>
                      )}
                    </Grid>
