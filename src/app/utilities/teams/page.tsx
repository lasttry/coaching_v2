  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
        <Typography variant="h5">{t('team.management')}</Typography>
        <Button variant="contained" startIcon={<AddIcon />} onClick={() => setDialogOpen(true)}>
          {t('team.add') as string}
        </Button>
      </Box>
      ...
      >
        <DialogTitle>
          {t('athlete.manage') as string}{' '}
          {selectedTeam ? `${t('common.for') as string} ${selectedTeam.name}` : ''}
        </DialogTitle>
        <DialogContent>
          <Box sx={{ height: 400 }}>
            <DataGrid
