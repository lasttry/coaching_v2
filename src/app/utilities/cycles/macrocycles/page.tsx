...
const MacroCyclesList = (): ReactElement => {
  ...
  const fetchMacroCycles = async (): Promise<void> => {
    try {
      ...
      setError(t('cycles.macrocycle.fetch.error'));
    }
  };
  ...
  const handleSave = async (): Promise<void> => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(t('cycles.macrocycle.save.error'));
      }
      ...
      setError(t('cycles.macrocycle.save.error'));
    }
  };

  const handleDelete = async (id: number): Promise<void> => {
    const confirmed = window.confirm(t('cycles.macrocycle.delete.confirm', { id }));
    if (!confirmed) return;
    ...
    try {
      ...
      if (response.ok) {
        setSuccess(t('cycles.macrocycle.delete.success', { id }));
        setMacroCycles((prevCycles) => prevCycles.filter((cycle) => cycle.id !== id));
      } else {
        setError(t('cycles.macrocycle.delete.error'));
      }
    } catch (err) {
  ...
  return (
    <div suppressHydrationWarning={true}>
      <PageContainer
        ...
        description={t('cycles.macrocycle.description')}
      >
        <h1>{t('cycles.macrocycle.title')}</h1>
        <Button
          variant="contained"
          color="primary"
          onClick={() => {
        ...
        {/* MacroCycles Table */}
        <DashboardCard title={t('cycles.macrocycle.title')}>
      ...
      <Dialog open={dialogOpen} onClose={closeOverlay} maxWidth="sm" fullWidth>
        <DialogTitle>
          {selectedMacrocycle ? t('cycles.macrocycle.edit') : t('cycles.macrocycle.add')}
        </DialogTitle>
        <DialogContent>
