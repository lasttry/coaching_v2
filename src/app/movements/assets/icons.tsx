    ...
    >
      ...
      <g>
        <use
          href="#path"
          fill="#cccccc"
          transform="matrix(1,0,0,1,1,0.5)"
        ></use>
...
export const BallIcon: React.FC<{ onClick: () => void }> = React.memo(
  ({ onClick }) => {
    const handleOnClick = () => {
      if (onClick) onClick();
    };
...
export const ConeIcon: React.FC<{ onClick: () => void }> = React.memo(
  ({ onClick }) => {
    const handleOnClick = () => {
      if (onClick) onClick();
    };
...
export const AreaIcon: React.FC<ChangeColorIconProps> = React.memo(
  ({ currentColor = '#003366', onColorChange }) => {
    ...
    const handleMoveLeft = () => {
      const newIndex =
        (currentColorIndex - 1 + DefaultColors.length) % DefaultColors.length;
